#!/usr/bin/env python3
"""
simulation3_4.py  —  extends simulation3_3 with two robustness tests

NEW 1: non-mediated pathway  (G → y direct, bypassing proteins)
  simulation3_3 routes ALL genetic effects through proteins (G→P→y).
  Real GWAS signals mostly bypass the measured proteins.
  Fix: split trait heritability into
    h2_med     (through causal proteins, same as before)
    h2_non_med (direct G→y, independent SNPs not in pQTL set)
  This makes the simulation more realistic and prevents artificially
  inflating the apparent PWAS signal.

NEW 2: tagging / co-regulation scenario
  Key risk in PWAS: a non-causal protein is genetically correlated
  (ρ = rho_tagging) with a causal rhythmic protein → inherits its
  GWAS signal as a false positive (tagging).
  This scenario tests: does the rhythmicity prior suppress tagging FPs?
  • "tagging" proteins: non-rhythmic, non-causal, ρ-correlated with causal
  • Reports: tagging_fp_rate_vanilla vs tagging_fp_rate_weighted
  • Expected: weighted < vanilla  (prior helps distinguish true from tagged)

Usage:
  python3 simulation3_4.py [--reps 200] [--out results_sim4]
  python3 simulation3_4.py --no-metacycle
"""
...
class Config:
    ...
    # ── NEW: split trait heritability into mediated + non-mediated ──────────
    h2_med         = 0.20   # h² mediated through causal proteins (G→P→y)
    h2_non_med     = 0.10   # h² via direct pathway (G→y, bypassing proteins)
    n_direct_snps  = 50     # independent SNPs with direct phenotype effect
    # residual noise = 1 - h2_med - h2_non_med = 0.70
    # (replaces the old h2_trait = 0.30)
    ...
    # ── NEW: tagging scenario ───────────────────────────────────────────────
    rho_tagging    = 0.50   # genetic correlation between causal and tagged protein
    n_tagging_pairs = 30    # number of (causal, tagged) pairs to test
    ...
    use_metacycle  = True
    use_real_ld    = True
    discretize_geno = True   # True → G ∈ {0,1,2}; False → keep continuous Z
...
def simulate_protein_time_matrix(cfg, G, data, rp, rng):
    ...
    return P_avg, pop_mean, A, nsd, phi, gv, has_signal
...
# =============================================================================
def inject_contrast_pairs(cfg, P_avg, gv, has_signal, snr, rng):
    lo = np.percentile(snr, cfg.low_amp_pct)
    hi = np.percentile(snr, cfg.high_amp_pct)
    lo_pool = [p for p in range(len(snr)) if snr[p] <= lo and has_signal[p]]
    hi_pool = [p for p in range(len(snr)) if snr[p] >= hi and has_signal[p]]
...
def select_causal(cfg, snr, scenario, has_signal, rng):
    """Causal selection based on TRUE SNR (A/σ), proxy for true rhythmicity.
    Breaks circularity: obs R² is a noisy estimate of SNR, not SNR itself."""
    idx = np.where(has_signal)[0]
    n   = min(cfg.n_causal, len(idx) - 1)
    if n <= 0:
        return np.array([], dtype=int)
    if scenario == "circadian":
        z    = (snr[idx] - snr[idx].mean()) / (snr[idx].std() + 1e-9)
...
def make_phenotype(cfg, P_avg, snr, scenario, has_signal, rng, G_direct=None,
                   fixed_causal=None):
    """
    Generate phenotype y with mediated + non-mediated pathways.

    G_direct : (N × n_direct_snps) independent SNPs for direct G→y effects.
               If None, h2_non_med is folded into residual noise.
    fixed_causal : if provided, skip select_causal and use this array directly
                   (used for the tagging scenario).
    """
    if fixed_causal is not None:
        causal = fixed_causal
    else:
        causal = select_causal(cfg, snr, scenario, has_signal, rng)

    if scenario == "null" or len(causal) == 0:
        return rng.standard_normal(P_avg.shape[0]), causal

    # ── Mediated signal: G → P → y ─────────────────────────────────────────
    gamma   = rng.normal(0.0, 1.0, len(causal))
    sig_med = (P_avg[:, causal] * gamma).sum(1)
    sig_med = (sig_med - sig_med.mean()) / (sig_med.std() + 1e-9)

    # ── Non-mediated signal: G → y (direct) ────────────────────────────────
    h2_non_med = cfg.h2_non_med
    if G_direct is not None and h2_non_med > 0:
        gamma_d    = rng.normal(0.0, 1.0, G_direct.shape[1])
        sig_direct = G_direct @ gamma_d
        sig_direct = (sig_direct - sig_direct.mean()) / (sig_direct.std() + 1e-9)
    else:
        sig_direct = np.zeros(cfg.N)
        h2_non_med = 0.0

    h2_med   = cfg.h2_med
    h2_resid = max(0.0, 1.0 - h2_med - h2_non_med)

    y = (np.sqrt(h2_med)     * sig_med
       + np.sqrt(h2_non_med) * sig_direct
       + np.sqrt(h2_resid)   * rng.standard_normal(cfg.N))
    return y, causal


def inject_tagging_pairs(cfg, P_avg, gv, has_signal, r2, rng):
    """
    Build (causal, tagged) protein pairs for the tagging scenario.

    causal  : rhythmic (high R²), has genetic signal → truly affects phenotype
    tagged  : non-rhythmic (low R²), genetically correlated with causal via
              shared genetic component (ρ = cfg.rho_tagging)

    The tagged protein's protein level:
        P_avg[:, tagged] = ρ × gv_causal + sqrt(1-ρ²) × noise

    Because it shares genetic variance with the causal protein, it inherits
    a spurious PWAS Z-score when y is generated from the causal protein.
    Question: does the rhythmicity prior suppress this false positive?
    """
    lo_pool = [p for p in range(P_avg.shape[1])
               if r2[p] <= np.percentile(r2, cfg.low_amp_pct) and has_signal[p]]
    hi_pool = [p for p in range(P_avg.shape[1])
               if r2[p] >= np.percentile(r2, cfg.high_amp_pct) and has_signal[p]]
    rng.shuffle(lo_pool); rng.shuffle(hi_pool)
    n = min(cfg.n_tagging_pairs, len(lo_pool), len(hi_pool))

    pairs = []
    for k in range(n):
        p_causal = hi_pool[k]   # rhythmic, will be made causal
        p_tagged = lo_pool[k]   # non-rhythmic, will receive correlated genetics

        gv_c = gv[:, p_causal].copy()
        gv_c = gv_c / (gv_c.std() + 1e-9)

        rho   = cfg.rho_tagging
        noise = rng.standard_normal(cfg.N)
        gv_t  = rho * gv_c + np.sqrt(1 - rho ** 2) * noise

        col   = (np.sqrt(cfg.h2_pqtl) * gv_t
                 + np.sqrt(1 - cfg.h2_pqtl) * rng.standard_normal(cfg.N))
        P_avg[:, p_tagged] = (col - col.mean()) / (col.std() + 1e-9)

        pairs.append((p_causal, p_tagged))
    return pairs


def evaluate_tagging(cfg, Z, obs_r2, tagging_pairs, return_detail=False):
    """
    For each (causal, tagged) pair:
      tagging FP (vanilla)  : tagged ranks ABOVE causal in vanilla PWAS
                              → method falsely promotes the non-causal protein
      tagging FP (weighted) : tagged ranks ABOVE causal after weighting
                              → should be lower if the prior works correctly

    Lower tagging_fp_rate_weighted means the prior suppresses false positives.
    """
    ...
    records = []
    for pid, (p_causal, p_tagged) in enumerate(tagging_pairs):
        records.append({
            "pair":                  pid,
            "r2_causal":             float(obs_r2[p_causal]),
            "r2_tagged":             float(obs_r2[p_tagged]),
            "vanilla_Z_causal":      float(np.abs(Z)[p_causal]),
            "vanilla_Z_tagged":      float(np.abs(Z)[p_tagged]),
            "vanilla_rank_causal":   int(van_rank[p_causal]),
            "vanilla_rank_tagged":   int(van_rank[p_tagged]),
            "weighted_rank_causal":  int(wtd_rank[p_causal]),
            "weighted_rank_tagged":  int(wtd_rank[p_tagged]),
            # True = tagged wrongly outranks causal (false positive)
            "vanilla_fp":   van_rank[p_tagged] < van_rank[p_causal],
            # True = weighted correctly ranks causal above tagged
            "weighted_ok":  wtd_rank[p_causal] < wtd_rank[p_tagged],
        })
    detail = pd.DataFrame(records)
    summary = {
        "n_pairs":                 len(tagging_pairs),
        "rho":                     cfg.rho_tagging,
        "mean_r2_causal":          float(detail["r2_causal"].mean()),
        "mean_r2_tagged":          float(detail["r2_tagged"].mean()),
        "tagging_fp_rate_vanilla": float(detail["vanilla_fp"].mean()),
        "tagging_fp_rate_weighted":float((~detail["weighted_ok"]).mean()),
        "tagging_reduction":       float(detail["vanilla_fp"].mean()
                                         - (~detail["weighted_ok"]).mean()),
    }
    if return_detail:
...
def evaluate_pairs(cfg, Z, obs_r2, pairs, snr, return_detail=False):
    n_proteins = len(Z)
    van_rank   = pd.Series(np.abs(Z)).rank(ascending=False).to_numpy()
    w          = make_weight(obs_r2, cfg.tau)
    wtd_score  = np.abs(Z) * np.sqrt(w)
    ...
    summary = {
        ...
        "mean_snr_lo":           float(detail["snr_lo"].mean()),
        "mean_snr_hi":           float(detail["snr_hi"].mean()),
...
def run_simulation(cfg, data, rp):
    ...
    P_avg, pop_mean, A, nsd, phi, gv, has_signal = simulate_protein_time_matrix(
        cfg, G, data, rp, rng)
    snr = A / (nsd + 1e-9)   # 真实信噪比：E[R²] ≈ snr²/(snr²+2)
    print(f"  P_avg shape: {P_avg.shape}  "
          f"(circadian component averaged out → input to PWAS)")
    print(f"  SNR range: [{snr.min():.3f}, {snr.max():.3f}], "
          f"median {np.median(snr):.3f}")

    print("Running MetaCycle on population-mean profiles → R² ...")
    r2 = compute_r2(cfg, pop_mean, rp["timepoints"])
    corr_r2_snr = float(np.corrcoef(r2, snr)[0, 1])
    print(f"  R² range: [{r2.min():.3f}, {r2.max():.3f}], "
          f"median {np.median(r2):.3f}")
    print(f"  Corr(R², true SNR A/σ): {corr_r2_snr:.3f}  "
          f"← R² is a NOISY proxy of true SNR")

    # ── Non-mediated SNPs (independent of pQTL, simulated once) ─────────────
    print(f"Simulating {cfg.n_direct_snps} non-mediated direct SNPs "
          f"(h2_non_med={cfg.h2_non_med}) ...")
    maf_direct = rng.uniform(0.05, 0.45, cfg.n_direct_snps)
    G_direct   = rng.binomial(2, maf_direct, size=(cfg.N, cfg.n_direct_snps)).astype(float)
    G_direct   = (G_direct - G_direct.mean(0)) / (G_direct.std(0) + 1e-9)
    print(f"  h2_med={cfg.h2_med}, h2_non_med={cfg.h2_non_med}, "
          f"residual={1-cfg.h2_med-cfg.h2_non_med:.2f}")

    print("Injecting matched pairs (same genetic effect, different SNR) ...")
    pairs = inject_contrast_pairs(cfg, P_avg, gv, has_signal, snr, rng)
    print(f"  {len(pairs)} pairs  "
          f"(SNR_lo ≤ {np.percentile(snr, cfg.low_amp_pct):.3f}, "
          f" SNR_hi ≥ {np.percentile(snr, cfg.high_amp_pct):.3f})")

    print("Injecting tagging pairs (genetic correlation ρ="
          f"{cfg.rho_tagging}) ...")
    tagging_pairs = inject_tagging_pairs(cfg, P_avg, gv, has_signal, r2, rng)
    print(f"  {len(tagging_pairs)} tagging pairs  "
          f"(causal=rhythmic, tagged=non-rhythmic, correlated)")

    scenarios = ["circadian", "random", "null"]
    rows, pair_results, tagging_results = [], [], []
    snapshot = snapshot_pairs = None

    print(f"Running {cfg.n_reps} reps × {len(scenarios)} scenarios + tagging ...")
    for sc in scenarios:
        for rep in range(cfg.n_reps):
            rng_rep   = np.random.default_rng(cfg.seed + 1000 * rep + hash(sc) % 997)
            y, causal = make_phenotype(cfg, P_avg, snr, sc, has_signal, rng_rep,
                                       G_direct=G_direct)
            Z         = pwas_z(P_avg, y)
            d, av, aw = compute_delta_auc(Z, r2, causal, Pn, cfg.tau)
            rows.append((sc, rep, d, av, aw))

            if sc == "circadian":
                if rep == 0:
                    sp, detail_p = evaluate_pairs(
                        cfg, Z, r2, pairs, snr, return_detail=True)
                    pair_results.append(sp)
                    snapshot_pairs = detail_p
                    w_snap = make_weight(r2, cfg.tau)
                    ...
                    pair_results.append(
                        evaluate_pairs(cfg, Z, r2, pairs, snr))

    # ── Tagging scenario: causal = rhythmic proteins from tagging pairs ──────
    print(f"Running {cfg.n_reps} reps for tagging scenario ...")
    tagging_causal = np.array([p_causal for p_causal, _ in tagging_pairs])
    snap_tagging   = None
    for rep in range(cfg.n_reps):
        rng_rep = np.random.default_rng(cfg.seed + 1000 * rep + hash("tagging") % 997)
        # Phenotype driven ONLY by causal (rhythmic) proteins in tagging pairs
        y, _ = make_phenotype(cfg, P_avg, snr, "circadian", has_signal, rng_rep,
                              G_direct=G_direct, fixed_causal=tagging_causal)
        Z     = pwas_z(P_avg, y)

        if rep == 0:
            ts, detail_t = evaluate_tagging(cfg, Z, r2, tagging_pairs,
                                            return_detail=True)
            tagging_results.append(ts)
            snap_tagging = detail_t
        else:
            tagging_results.append(evaluate_tagging(cfg, Z, r2, tagging_pairs))
    ...
    keys = ("mean_vanilla_z_diff", "weighted_correct_rate",
            "mean_snr_lo", "mean_snr_hi", "mean_r2_lo", "mean_r2_hi",
            "mean_rank_improvement")
    pair_summ = {k: float(np.mean([x[k] for x in pair_results]))
                 for k in keys} if pair_results else {}

    tag_keys = ("tagging_fp_rate_vanilla", "tagging_fp_rate_weighted",
                "tagging_reduction", "mean_r2_causal", "mean_r2_tagged", "rho")
    tag_summ = {k: float(np.mean([x[k] for x in tagging_results]))
                for k in tag_keys} if tagging_results else {}

    return dict(df=df, summary=summ, pair_summary=pair_summ,
                tagging_summary=tag_summ, tagging_detail=snap_tagging,
...
def print_results(res, cfg):
    ...
    if res["pair_summary"]:
        ...
        print(f"  Mean true SNR  lo / hi: "
              f"{c['mean_snr_lo']:.3f} / {c['mean_snr_hi']:.3f}")
    ...
    if res.get("tagging_summary"):
        t = res["tagging_summary"]
        print(f"\n{'='*62}")
        print(f"RESULTS  Tagging scenario  (ρ = {t['rho']:.2f})")
        print(f"{'='*62}")
        print(f"  Pairs: causal R²={t['mean_r2_causal']:.3f} "
              f"(rhythmic)  |  tagged R²={t['mean_r2_tagged']:.3f} (non-rhythmic)")
        print(f"  Tagging FP rate — vanilla PWAS : {t['tagging_fp_rate_vanilla']:.3f}"
              f"  (tagged wrongly outranks causal)")
        print(f"  Tagging FP rate — weighted PWAS: {t['tagging_fp_rate_weighted']:.3f}"
              f"  (should be lower)")
        print(f"  FP reduction by weighting       : "
              f"{t['tagging_reduction']:+.3f}  "
              f"({'✓ prior suppresses tagging' if t['tagging_reduction'] > 0 else '! no improvement'})")
...
def main():
    ...
    ap.add_argument("--out",           default=os.path.join(HERE, "results_sim4"))
    ...
    if res.get("tagging_summary"):
        pd.DataFrame([res["tagging_summary"]]).to_csv(
            os.path.join(args.out, "tagging_summary.csv"), index=False)
    if res.get("tagging_detail") is not None:
        res["tagging_detail"].to_csv(
            os.path.join(args.out, "tagging_detail.csv"), index=False)
