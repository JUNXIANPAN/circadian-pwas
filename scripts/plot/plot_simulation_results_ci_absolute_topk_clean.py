...
4) Absolute baseline-vs-weighted Top-k comparison figure is added.
5) Replicate jitter points are removed for cleaner presentation.
6) Tail-error plot uses log scale for better visibility.
...
Example
-------
python scripts/plot/plot_simulation_results_ci_absolute_topk_clean.py \
  --base_dir work/sim_result_3322 \
  ...
  --f sim_F_global_null \
  --outdir work/sim_plots_ci_clean
"""
...
def plot_baseline_vs_weighted_topk_ci(master: pd.DataFrame, result_tables: Dict[str, pd.DataFrame], outdir: Path) -> None:
    df = master.loc[master["scenario_id"].isin(["A", "B", "C", "D", "E"])].copy()
    x = np.arange(len(df))
    width = 0.38
    ...
    for row in df.itertuples(index=False):
        rep = result_tables[row.scenario_id]
        bmean, _, bci = mean_se_ci(rep["baseline_topk_hits"].to_numpy(dtype=float))
        wmean, _, wci = mean_se_ci(rep["weighted_topk_hits"].to_numpy(dtype=float))
        baseline_means.append(bmean)
        baseline_cis.append(bci)
        weighted_means.append(wmean)
        weighted_cis.append(wci)
    ...
    plt.title("Baseline vs weighted Top-k hits (95% CI)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(outdir / "figure5_baseline_vs_weighted_topk_ci.png", dpi=300)
    plt.close()
...
def main() -> None:
    ...
    plot_baseline_vs_weighted_topk_ci(master, result_tables, outdir)
