...
def load_ld_matrix(ld_file: Path, max_snps: Optional[int] = None) -> np.ndarray:
    """
    Load LD matrix from .npz.

    Supports several common formats:
      1. Dense matrix stored as arr_0 / ld / LD / corr / R / matrix
      2. Sparse CSR saved by scipy.sparse.save_npz:
           data, indices, indptr, shape
      3. Sparse COO-like npz:
           row, col, data, shape
      4. Generic first 2D array found in the npz

    Returns a dense, symmetric, stabilized correlation matrix.
    """
    ...
    # Case 1: common dense matrix keys
    ...
    # Case 2: scipy sparse CSR .npz format
    # scipy.sparse.save_npz usually stores: indices, indptr, format, shape, data
    ...
    # Case 3: COO-like sparse format
    if mat is None and {"data", "row", "col", "shape"}.issubset(set(keys)):
        try:
            from scipy import sparse
            shape = tuple(data["shape"].tolist())
            coo = sparse.coo_matrix((data["data"], (data["row"], data["col"])), shape=shape)
            mat = coo.toarray()
        except Exception as e:
            raise ValueError(
    ...
    # Case 4: fallback first 2D numeric array
    if mat is None:
        for k in keys:
            arr = data[k]
            if isinstance(arr, np.ndarray) and arr.ndim == 2:
    ...
    if max_snps is not None and mat.shape[0] > max_snps:
        mat = mat[:max_snps, :max_snps]

    # Clean and stabilize
    mat = np.nan_to_num(mat, nan=0.0, posinf=0.0, neginf=0.0)
    mat = (mat + mat.T) / 2.0

    # If diagonal is all zero or missing, set to one.
    np.fill_diagonal(mat, 1.0)

    # Clip extreme numerical artifacts
    mat = np.clip(mat, -1.0, 1.0)

    # Add jitter for positive semidefinite stability
...
def main():
    ...
    parser.add_argument("--max-ld-snps", type=int, default=1000)
    parser.add_argument("--protein-noise-sd", type=float, default=0.2)
    parser.add_argument("--alpha-r", type=float, default=0.25)
    parser.add_argument("--alpha-a", type=float, default=0.25)
    parser.add_argument("--alpha-p", type=float, default=0.25)
    parser.add_argument("--alpha-c", type=float, default=0.25)
    parser.add_argument("--target-phase-hour", type=float, default=9.0)
    parser.add_argument("--main-noise-sigma", type=float, default=1.75)
    parser.add_argument("--main-shrink-to-one", type=float, default=0.5)
    parser.add_argument("--main-gamma", type=float, default=2.0)
