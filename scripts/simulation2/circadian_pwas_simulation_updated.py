...
def make_wrong_phase_observed_weight(ann: pd.DataFrame, params: WeightParams, rng: np.random.Generator) -> np.ndarray:
    wrong = WeightParams(0.0, 0.0, 1.0, 0.0, params.tau, (params.target_phase_hour + 12.0) % 24.0, params.observed_noise_sigma, params.observed_shrink_to_one)
...
def summarize_results(raw: pd.DataFrame) -> pd.DataFrame:
    group_cols = ["experiment", "scenario", "n_proteins", "n_causal", "trait_h2", "causal_enrichment_gamma", "observed_weight_noise_sigma", "observed_weight_shrink_to_one", "alpha_r", "alpha_a", "alpha_p", "alpha_c", "tau"]
...
def main():
    ...
    parser.add_argument("--tau", type=float, default=4.0, help="Prior contrast parameter in w*=exp(tau*S). Recommended range: 3-5.")
    parser.add_argument("--target-phase-hour", type=float, default=9.0)
    parser.add_argument("--main-noise-sigma", type=float, default=0.3)
    parser.add_argument("--main-shrink-to-one", type=float, default=0.3)
    ...
    all_raw = []
    base_wp = WeightParams(args.alpha_r, args.alpha_a, args.alpha_p, args.alpha_c, args.tau, args.target_phase_hour, args.main_noise_sigma, args.main_shrink_to_one)
    ...
    if "prior" in blocks:
        for noise, shrink in [(0.75, 0.4), (1.00, 0.4), (1.25, 0.4), (1.50, 0.5), (1.75, 0.5), (2.00, 0.55), (2.25, 0.6), (2.50, 0.65)]:
            wp = WeightParams(args.alpha_r, args.alpha_a, args.alpha_p, args.alpha_c, args.tau, args.target_phase_hour, noise, shrink)
        ...
        for ar in [0.10, 0.25, 0.40]:
            for aa in [0.10, 0.25, 0.40]:
                for ap in [0.10, 0.25, 0.40]:
                    ac = 1.0 - ar - aa - ap
                    if ac < 0: continue
                    wp = WeightParams(ar, aa, ap, ac, args.tau, args.target_phase_hour, args.main_noise_sigma, args.main_shrink_to_one)
    ...
    if all_raw:
        ...
        keep = ["experiment", "scenario", "n_reps", "n_proteins", "n_causal", "trait_h2", "causal_enrichment_gamma", "observed_weight_noise_sigma", "observed_weight_shrink_to_one", "tau", "weight_corr_log_latent_observed", "ordinary_auc", "weighted_auc", "delta_auc", "ordinary_pr_auc", "weighted_pr_auc", "delta_pr_auc"]
